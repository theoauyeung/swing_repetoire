...
**Support-constrained α (UNRESOLVED).** The support envelope is each unit's OBSERVED shape percentiles (wide), while the shapes being blended are FITTED values whose situational component is far narrower — blending to α=2 almost never leaves the envelope: 406 of 471 units (88.5%) peg at the grid maximum α=2.0. `alpha_star_supported` is therefore NOT a recommendation and must never be documented as one. The only prescriptive quantity that survives is `marginal_runs_per_alpha` — the local slope at current behaviour (mean +1.69 runs per α unit).
