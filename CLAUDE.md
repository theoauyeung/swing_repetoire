...
**Section 2 — Overall performance OLS (headline, wOBA outcome):** season wOBA regressed on composite `adjustability` (+ `swing_plus` + `repertoire_pctile`), z-scored, clustered SE by batter_id. N=467 units / 446 batters. wOBA comes from `fetch_woba()` — Baseball Savant custom leaderboard, PA-weighted 2024-25, ≥200 PA, cached to `data/batter_woba_savant_2425.parquet` (mean ≈0.309). Do NOT rebuild wOBA from `linear_weights.csv`: those weights are centered for the xRV model, not the official wOBA scale, and a rebuild lands ~0.06 low.

wOBA became the headline outcome on 2026-08-03 because a two-strike penalty shares its conditioning set with `adj_count` (treatment estimated from count behavior, outcome measured on two-strike swings); that circularity demoted the penalty OLS to Section 2b. **Key findings:** composite `adjustability` is null on wOBA (θ=−0.042, p=0.35); only `adj_count` registers (θ=+0.100, p=0.014). Raw composite-vs-wOBA r=−0.137 is a talent confound — partial r=−0.047 (p=0.31) after Swing+, which is itself the dominant predictor (r=+0.44).

**Section 2b — Situational penalty OLS (diagnostic):** each penalty regressed on its corresponding axis, same controls and structure. `adj_pitch` excluded — pitch type is reactive, not a pre-pitch volitional lever. `adj_count` θ=+0.145 (SE=0.040, t=3.64, p=0.0003) on `twostrike_rv_penalty`; composite θ=+0.137 (p=0.004). Game-state (any runner, p=0.13) and platoon (p=0.83) null. Read these as mechanism, not payoff — the count coefficient here runs ~1.5× the wOBA estimate, roughly what shared conditioning would inflate it by.

Use composite `adjustability` (not `adj_count`) as the headline treatment in plots and regressions; report the axis breakdown alongside it.
